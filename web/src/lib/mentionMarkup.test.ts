...
const IDS: IdentityDisplayMap = {
  ...
  codex: { display: "Codex", kind: "agent" },
  agentparty: { display: "AgentParty", kind: "agent" },
  "example.com": { display: "Example Agent", kind: "agent" },
...
describe("mention 美化：真实管线集成（#131）", () => {
  ...
  it("无 scheme URL、Unicode email 与 npm scope 都不美化成 mention", () => {
    for (const source of [
      "github.com/@alice/repo",
      "请看github.com/@alice/repo",
      "请看：github.com/@alice/repo",
      "测试@example.com",
      "install @agentparty/shared",
    ]) {
      expect(R(source), source).not.toContain("ap-mention");
    }
  });
...
describe("mention 美化：过程断言（renderer 是否被调用、入参是什么）", () => {
  ...
  it("无 scheme/CJK URL、Unicode email 与 npm scope：mention renderer 一次都不被调用", () => {
    const s = spy(IDS);
    s.render("请看github.com/@alice/repo；测试@example.com；install @agentparty/shared");
    expect(s.calls).toEqual([]);
  });
});
